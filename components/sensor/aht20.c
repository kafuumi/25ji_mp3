...
#define INIT_CHECK() ESP_RETURN_ON_FALSE(aht20_dev_handle, ESP_FAIL, TAG, "aht20 is not initialized!")
...
esp_err_t aht20_read_temperature_humidity(float *temperature, float *humidity) {
    ...
    if (ESP_OK != err) {
        ESP_LOGE(TAG, "send AC cmd to aht20 fail: %d(%s)", err, esp_err_to_name(err));
        return err;
    }
    ...
    ESP_LOGD(TAG, "read data success, raw: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x", rb[0], rb[1], rb[2], rb[3], rb[4], rb[5]);
    ...
    if (temperature)
        *temperature = (float)raw_temperature * 200 / 1048576 - 50; // raw/2^20 * 200 - 50
    ESP_LOGD(TAG,
...
esp_err_t aht20_read_temperature(float *temperature) { return aht20_read_temperature_humidity(temperature, NULL); }

esp_err_t aht20_read_humidity(float *humidity) { return aht20_read_temperature_humidity(NULL, humidity); }
...
esp_err_t aht20_init(i2c_master_bus_handle_t bus_handle) {
    ...
    ESP_RETURN_ON_FALSE(bus_handle, ESP_ERR_INVALID_ARG, TAG, "i2c master bus handle is null");
    ...
    esp_err_t err = i2c_master_bus_add_device(bus_handle, &dev_cfg, &aht20_dev_handle);
    if (ESP_OK != err) {
        ESP_LOGE(TAG, "failed to add device to i2c bus, err:%d(%s)", err, esp_err_to_name(err));
        return err;
    }
