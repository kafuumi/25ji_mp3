...
static uint8_t gpio_and_delay_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
    ...
    case U8X8_MSG_GPIO_RESET:
        // reset
        ESP_LOGD(TAG, "gpio: reset, arg_ind=%d", arg_int);
        break;
    case U8X8_MSG_GPIO_AND_DELAY_INIT:
        // init
        ESP_LOGD(TAG, "gpio: gpio and delay init, arg_int=%d", arg_int);
        break;
    case U8X8_MSG_DELAY_MILLI:
        ESP_LOGD(TAG, "gpio: delay %d ms", arg_int);
...
static uint8_t u8x8_byte_hw_i2c(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
    ...
    case U8X8_MSG_BYTE_INIT:
        ...
        ESP_LOGD(TAG, "i2c: initialize, arg_int=%d", arg_int);
        break;
    case U8X8_MSG_BYTE_START_TRANSFER:
        ESP_LOGD(TAG, "i2c: transfer start");
        // reset buffer size
        count = 0;
        break;
    case U8X8_MSG_BYTE_SEND:
        ESP_LOGD(TAG, "i2c: transfer bytes, size=%d", arg_int);
    ...
    case U8X8_MSG_BYTE_END_TRANSFER:
        ESP_LOGD(TAG, "i2c: transfer end, total bytes=%d", count);
        if (0 == count) {
            ESP_LOGD(TAG, "i2c: total bytes is 0, do not send");
        } else {
            err = i2c_master_transmit(i2c_dev_handle, i2c_buf, count, i2c_timeout);
        }
        break;
    default:
        ESP_LOGD(TAG, "i2c: msg=%d, arg_int=%d, arg_ptr=0x%p", msg, arg_int, arg_ptr);
        break;
    }
    if (ESP_OK != err) {
